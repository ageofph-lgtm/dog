...
// Contexto global para o Dashboard aceder ao user/permissions do Layout
...
export default function Layout({ children }) {
  ...
  const loadUser = async () => {
    try {
      // Primeiro tenta localStorage (sessão persistida pelo Dashboard)
      const saved = localStorage.getItem('watcher_profile');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed?.perfil) { setUser(parsed); setIsAuthenticated(true); setIsLoadingUser(false); return; }
      }
  ...
  return (
    ...
    <div style={{
      ...
      paddingBottom: `${FOOTER_H + (isBannerActive ? BANNER_H : 0)}px`,
    }}>

      {/* ── PWA Banner ─────────────────────────────────────────────────── */}
      {isBannerActive && (
        <div style={{ position: 'fixed', bottom: `${FOOTER_H}px`, left: 0, right: 0, zIndex: 200, height: `${BANNER_H}px`, background: `linear-gradient(135deg, ${T.pink}, ${T.purple})`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
      ...
      {/* ── CONTEÚDO (sem paddingTop — sem nav no topo) ─────────────── */}
      ...
      <footer style={{
        ...
        boxShadow: isDark ? '0 -2px 20px rgba(0,0,0,0.5)' : '0 -2px 12px rgba(0,0,0,0.06)',
        ...
        {user && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {/* Avatar */}
            <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: `linear-gradient(135deg, ${T.pink}, ${T.purple})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '13px', color: 'white', fontFamily: 'monospace' }}>
            ...
            {/* Logout */}
            <button onClick={handleLogout} title="Sair / Trocar perfil" style={{ background: 'none', border: `1px solid ${theme.border}`, borderRadius: '6px', cursor: 'pointer', padding: '5px 8px', color: theme.muted, display: 'flex', alignItems: 'center', gap: '4px' }}>
              <LogOut size={13} />
